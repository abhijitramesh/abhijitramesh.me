...
<Latex>The perceptron is the building block of a neural network. It has inputs from $x_1$ to $x_m$ which are multiplied to its corresponding weights $w_1$ to $w_m$ which the results of which are summed together and passed through a non-linear activation function giving us the output $\hat \bgroup y \egroup$.</Latex>
<Image
  alt={`Image`}
...
<Latex>Where X is a vector which has all the input values from $x_1$ to $x_m$</Latex>
<br/>
<Latex>W is a vector which has all the weight values from $w_1$ to $w_m$</Latex>
<br/>
<Latex>$w_0$ is the bias term</Latex>
...
<Latex>$x_1$ the number of lectures that we attended and </Latex>

<Latex>$x_2$ hours spent on the final project</Latex>
...
<Latex>Let's pass this to a new perceptron with two inputs $x_1$ and $x_2$ and some hidden layers then finally leading upto our output.</Latex>
<Image
  alt={`Image`}
...
<ol>
  <li><Latex> Pick a Random point ($w_0$,$w_1$)</Latex></li>
  <li> Computer the gradient of that point with respect to weight</li>
<li> The gradient tells us the direction to go up the loss but we need to do the opposite so we travel in the opposite direction in small steps.</li>
  <li>We find the gradient there and repeat the process until we converge.</li>
</ol>
...
[https://github.com/abhijitramesh/learning-introtodeeplearning/blob/main/Part1_TensorFlow.ipynb](https://github.com/abhijitramesh/learning-introtodeeplearning/blob/main/Part1_TensorFlow.ipynb)
