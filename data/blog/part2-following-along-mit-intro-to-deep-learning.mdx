...
- <Latex> $h_t$  is the cell state </Latex>
- <Latex> $f_w$ is the function with weights W </Latex>
- <Latex> $x_t$ is the input </Latex>
- <Latex> $h_\bgroup t-1 \egroup$ is the old state</Latex>
...
- <Latex>a $h_\bgroup t-1 \egroup $ which is coming from the previous state</Latex>
- <Latex> An input $x_t$</Latex>
- <Latex>After being multiplied with their respective weights they are passed through a tanh activation</Latex>
- <Latex>An output $y_t$</Latex>
- <Latex>A hidden layer update $h_t$</Latex>
